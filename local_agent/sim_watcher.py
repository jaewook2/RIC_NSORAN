...
#pip3 install statsd (4.0.1) joblib (1.1.1, 1.4.2pip ) numpy watchdog ( 2.3.1 == 6.0.0)statsd-tags (3.2.1.post1)
#pip3 install statsd-telegraf (3.2.1.post1) --user
    ...
    def _send_to_telegraf(self, ue:int, values:List, fields:List, file_type:int):
        ...
        for field in fields:
            ...
            if 'UEID' not in field:
                ...
                #pipe.timing('stats.timed',timestamp)
                pipe.incr('foo')
                pipe.send()
                i+=1
                continue
